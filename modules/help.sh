...
show_help_section_install() {
    ...
    echo -e "${CYAN}install-health${NC}       - ${T_HELP_CMD_INSTALL_HEALTH:-Install health watchdog}"
    echo -e "${CYAN}install-supply-chain${NC} - ${T_HELP_CMD_INSTALL_SUPPLY:-Initialize supply-chain verification}"
    echo -e "${CYAN}install-lkg${NC}          - ${T_HELP_CMD_INSTALL_LKG:-Save LKG blocklist cache}"
    echo -e "${CYAN}install-ipv6${NC}         - ${T_HELP_CMD_INSTALL_IPV6:-Configure IPv6 privacy}"
...
show_help_section_advanced() {
    ...
    echo -e "${CYAN}benchmark-dns${NC}      - ${T_HELP_CMD_BENCHMARK_DNS:-Run DNS performance benchmark}"
    echo -e "${CYAN}benchmark-cache${NC}    - ${T_HELP_CMD_BENCHMARK_CACHE:-Run cache performance test}"
    echo -e "${CYAN}benchmark-blocklist${NC} - ${T_HELP_CMD_BENCHMARK_BLOCKLIST:-Run blocklist performance test}"
    echo -e "${CYAN}benchmark-all${NC}      - ${T_HELP_CMD_BENCHMARK_ALL:-Run comprehensive benchmark suite}"
    echo -e "${CYAN}benchmark-show${NC}     - ${T_HELP_CMD_BENCHMARK_SHOW:-Show benchmark reports}"
    echo -e "${CYAN}benchmark-compare${NC}  - ${T_HELP_CMD_BENCHMARK_COMPARE:-Compare benchmark results}"
