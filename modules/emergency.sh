...
panic_bypass() {
    ...
    [[ -f "$PANIC_STATE_FILE" ]] && { log_warning "Already in panic mode. Use 'panic-restore' to exit."; return 1; }
    ...
    if [[ -t 0 && -t 1 ]]; then
        echo -n "Continue? [y/N]: "
        read -r answer
        [[ ! "$answer" =~ ^[Yy]$ ]] && { log_info "Cancelled."; return 0; }
    fi
...
panic_restore() {
    ...
    [[ ! -f "$PANIC_STATE_FILE" ]] && { log_info "Not in panic mode."; return 0; }
...
emergency_refuse() {
    ...
    [[ ! -f "$corefile" ]] && { log_error "CoreDNS not installed"; return 1; }
