...
location_is_trusted() {
    ...
    grep -qxF "$ssid" "$TRUSTED_SSIDS_FILE" 2>/dev/null
}
...
location_remove_trusted() {
    local ssid="$1"
    [[ -z "$ssid" ]] && { log_error "Usage: location-remove-trusted <SSID>"; return 1; }
    [[ ! -f "$TRUSTED_SSIDS_FILE" ]] && { log_warning "No trusted SSID list found"; return 0; }
    if grep -qxF "$ssid" "$TRUSTED_SSIDS_FILE" 2>/dev/null; then
...
install_dnscrypt() {
    ...
    if ! id dnscrypt &>/dev/null; then
        log_info "Creating dedicated 'dnscrypt' user..."
        useradd -r -s /usr/bin/nologin -d /var/lib/dnscrypt dnscrypt
        log_success "User dnscrypt created"
    else
        log_success "User dnscrypt already exists"
    fi
    ...
    if [[ -z "$dnscrypt_port" ]]; then
        log_error "Unable to find a free port for DNSCrypt (range ${DNSCRYPT_PORT_DEFAULT}-5365)"
        return 1
    fi

    # Create configuration - MINIMAL VERSION
    log_info "Generating DNSCrypt configuration (resolver sources + minisign verification)..."
...
EOF
    ...
    log_info "Generating advanced configuration (optional)..."
...
EOF
    ...
    # Test configuration
    log_info "Validating TOML configuration..."
    if dnscrypt-proxy -config /etc/dnscrypt-proxy/dnscrypt-proxy.toml -check; then
        log_success "DNSCrypt configuration OK"
    else
        log_error "TOML configuration error - check logs"
        return 1
    fi
    ...
    if systemctl is-active --quiet dnscrypt-proxy; then
        log_success "DNSCrypt-Proxy is running"
    else
        log_error "DNSCrypt-Proxy failed to start - check: journalctl -u dnscrypt-proxy -n 50"
        return 1
    fi

    log_success "DNSCrypt module installed"
}
...
install_coredns() {
    ...
    log_info "Bootstrap DNS (CoreDNS forward -> DNSCrypt) during list downloads..."
...
EOF
    ...
    # Download blocklists
    log_info "Downloading blocklists (OISD + KADhosts + Polish Annoyance + HaGeZi Multi Light)..."
    {
        ...
        if [[ $(wc -l < "$tmp_block") -lt 1000 ]]; then
            ...
            chown root:coredns /etc/coredns/zones/blocklist.hosts /etc/coredns/zones/combined.hosts 2>/dev/null || true
            chmod 0640 /etc/coredns/zones/blocklist.hosts /etc/coredns/zones/combined.hosts 2>/dev/null || true
            log_success "Blocklist downloaded ($(wc -l < /etc/coredns/zones/blocklist.hosts) entries)"
        fi
    } || {
        log_warning "Failed to download blocklists - creating an empty file"
        touch /etc/coredns/zones/blocklist.hosts
...
EOF
    ...
    log_info "Configuring automatic blocklist updates..."
...
EOF
    ...
    if systemctl is-active --quiet coredns; then
        ...
        log_error "CoreDNS failed to start - check: journalctl -u coredns -n 50"
        return 1
    fi

    log_success "CoreDNS module installed"
}
...
adblock_show() {
    ...
    case "$which" in
        ...
        combined)
            ...
            ;;
        *)
            log_error "Usage: adblock-show custom|blocklist|combined"
            return 1
            ;;
    esac
}
...
adblock_query() {
    local domain="$1"
    if [[ -z "$domain" ]]; then
        log_error "Usage: adblock-query <domain>"
        return 1
    fi
...
adblock_add() {
    ...
    if grep -qE "^[0-9.:]+[[:space:]]+${domain}$" /etc/coredns/zones/custom.hosts 2>/dev/null; then
        log_info "Already present in custom.hosts: $domain"
    else
        printf '0.0.0.0 %s\n' "$domain" >> /etc/coredns/zones/custom.hosts
        log_success "Added to custom.hosts: $domain"
    fi
    adblock_rebuild
    adblock_reload
}
...
adblock_remove() {
    local domain="$1"
    if [[ -z "$domain" ]]; then
        log_error "Usage: adblock-remove <domain>"
        return 1
    fi
    if [[ ! -f /etc/coredns/zones/custom.hosts ]]; then
        log_warning "Missing /etc/coredns/zones/custom.hosts"
        return 0
    fi
    sed -i -E "/^[0-9.:]+[[:space:]]+${domain//./\.}([[:space:]]|
|$)/d" /etc/coredns/zones/custom.hosts || true
    log_success "Removed from custom.hosts (if present): $domain"
    adblock_rebuild
    adblock_reload
}
...
install_nftables() {
    ...
    log_info "Generating firewall rules..."
...
EOF
    ...
    then
        log_success "nftables syntax OK"
    else
        log_error "nftables syntax error"
        return 1
    fi
    ...
    log_success "NFTables module installed"
}
...
firewall_safe() {
    ...
    nft delete table inet citadel_dns 2>/dev/null || true
    nft delete table inet citadel_emergency 2>/dev/null || true
    nft -f /etc/nftables.d/citadel-dns-safe.nft || log_warning "Failed to load SAFE rules (check: sudo nft -c -f /etc/nftables.d/citadel-dns-safe.nft)"
    log_success "Firewall set to SAFE"
}
...
firewall_strict() {
    ...
    nft delete table inet citadel_dns 2>/dev/null || true
    nft delete table inet citadel_emergency 2>/dev/null || true
    nft -f /etc/nftables.d/citadel-dns-strict.nft || log_warning "Failed to load STRICT rules (check: sudo nft -c -f /etc/nftables.d/citadel-dns-strict.nft)"
    log_success "Firewall set to STRICT"
}
...
configure_system() {
    ...
    log_warning "This step changes system DNS (it may temporarily disrupt internet if DNS is not working)."
...
EOF
    ...
    # Lock resolv.conf
    log_info "Writing /etc/resolv.conf (Citadel++ DNS)..."
...
EOF
    ...
    log_info "Testing local DNS after switch..."
    if command -v dig >/dev/null 2>&1 && dig +time=2 +tries=1 +short google.com @127.0.0.1 >/dev/null 2>&1; then
        log_success "DNS OK (localhost)"
        if command -v nft >/dev/null 2>&1 && [[ -f /etc/nftables.conf ]]; then
            log_info "Enabling STRICT firewall (full DNS leak protection)..."
            firewall_strict 2>/dev/null || true
        fi
    else
        log_warning "DNS test failed - keeping SAFE firewall"
 ...
 restore_system() {
     ...
     log_success "Restored systemd-resolved and system DNS settings"
 }
...
install_all() {
    ...
    if command -v dig >/dev/null 2>&1; then
        ...
        log_warning "'dig' not found - skipping DNS/Adblock tests"
    fi

    echo ""
    log_info "Diagnostic tests:"
    echo "  1. DNS test:        dig +short google.com @127.0.0.1"
    echo "  2. Metrics test:    curl -s http://127.0.0.1:9153/metrics | grep coredns_"
    echo "  3. DNSCrypt logs:   journalctl -u dnscrypt-proxy -f"
    echo "  4. CoreDNS logs:    journalctl -u coredns -f"
    echo "  5. Firewall:        sudo nft list ruleset | grep citadel"
    echo "  6. Leak test:       dig @8.8.8.8 test.com (should be blocked)"
    echo ""
...
EOF
    systemctl restart coredns
    log_warning "DNS REFUSE MODE ACTIVE - all queries are rejected"
}
...
emergency_restore() {
    ...
    systemctl restart coredns
    log_success "Normal mode restored"
}
...
emergency_killswitch_on() {
    ...
    log_warning "Kill-switch ACTIVE - all DNS outside localhost is blocked"
}
...
smart_ipv6_detection() {
    ...
    if ping6 -c 1 -W 2 2001:4860:4860::8888 >/dev/null 2>&1; then
        ...
        log_warning "IPv6 unavailable - using IPv4 only"
    fi
    ...
    if [[ -f "$dnscrypt_config" ]]; then
        log_info "Updating DNSCrypt configuration for IPv6..."
        ...
        if grep -qE '^ipv6_servers[[:space:]]*=' "$dnscrypt_config"; then
            ...
            if [[ "$IPV6_AVAILABLE" == "true" ]]; then
                printf '\nipv6_servers = true\n' >> "$dnscrypt_config"
                log_success "Added ipv6_servers = true"
            else
                printf '\nipv6_servers = false\n' >> "$dnscrypt_config"
                log_success "Added ipv6_servers = false"
            fi
        fi
    ...
    if [[ -f "$coredns_corefile" ]]; then
        log_info "Updating CoreDNS configuration for IPv6..."
...
ipv6_privacy_on() {
    ...
    log_info "Enabling IPv6 Privacy Extensions (prefer temporary addresses)..."
...
ipv6_privacy_off() {
    ...
    log_info "Disabling IPv6 Privacy Extensions..."
...
ipv6_privacy_status() {
    ...
    echo ""
    echo "IPv6 addresses (global/temporary):"
...
install_citadel_top() {
    ...
    if ! command -v pacman >/dev/null 2>&1; then
        log_warning "pacman not found - cannot auto-install dashboard dependencies"
        return 1
    fi
...
EOF
    ...
    log_success "Dashboard installed: run 'citadel-top'"
}
...
install_editor_integration() {
    ...
    if ! command -v yay >/dev/null 2>&1; then
        log_warning "yay not found - cannot auto-install micro editor"
        return 1
    fi
...
EOF
    ...
    log_success "Editor integration installed: use 'citadel edit'"
}
...
optimize_kernel_priority() {
    ...
    if [[ ! -f /etc/arch-release ]]; then
        log_warning "This function is optimized for CachyOS/Arch Linux"
        return 1
    fi
...
EOF
    ...
    log_success "Kernel priority optimization enabled"
}
...
EOF
    ...
    log_success "DoH Parallel Racing configuration ready"
    log_info "To enable: sudo cp /etc/dnscrypt-proxy/dnscrypt-proxy-doh.toml /etc/dnscrypt-proxy/dnscrypt-proxy.toml"
}
...
fix_port_conflicts() {
    ...
    echo "Processes using port 53:"
    sudo ss -tulpn | grep :53
    
    echo ""
    log_info "Fix options:"
    echo "1. Stop avahi: sudo systemctl stop avahi-daemon"
    echo "2. Kill chromium processes: pkill chromium"
    echo "3. Use an alternative port for CoreDNS"
    
    # Option to use different port for CoreDNS
    read -p "Change CoreDNS port to 5354? (yes/no): " -r
    if [[ $REPLY =~ ^(tak|TAK|yes|YES|y|Y)$ ]]; then
        log_info "Switching CoreDNS port to 5354..."
        sudo sed -i 's/.:53 {/.:5354 {/g' /etc/coredns/Corefile
        sudo systemctl restart coredns
        log_success "CoreDNS is now listening on port 5354"
...
${GREEN}Tools v3.0${CYAN}Tools:${NC}
...
${CYAN}Documentation:${NC}
  Notes (PL):          /home/qguar/Cytadela/CITADEL++_NOTES.md
  Notes (EN):          /home/qguar/Cytadela/CITADEL++_NOTES_EN.md
...
safe_test_mode() {
    ...
    log_info "Running tests without interrupting internet connectivity..."
    
    # Test 1: Check dependencies
    log_info "Checking dependencies..."
    for cmd in dnscrypt-proxy coredns nftables; do
        if command -v "$cmd" >/dev/null; then
            echo "󰄬 $cmd available"
        else
            echo "✗ $cmd not found"
        fi
    done
    ...
    if [[ -f /etc/dnscrypt-proxy/dnscrypt-proxy.toml ]]; then
        if dnscrypt-proxy -config /etc/dnscrypt-proxy/dnscrypt-proxy.toml -check >/dev/null 2>&1; then
            echo "󰄬 DNSCrypt config OK"
        else
            echo "✗ DNSCrypt config FAILED"
        fi
    fi
    ...
    if ss -ln | grep -q ":53"; then
        echo "⚠ Port 53 is in use - you may need to stop systemd-resolved"
    else
        echo "󰄬 Port 53 is free"
    fi
    
    echo ""
    log_info "Safe test mode completed. Use 'install-all' for full installation"
}
